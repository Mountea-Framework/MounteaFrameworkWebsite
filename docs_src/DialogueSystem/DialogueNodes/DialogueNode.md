...
=== "C++"
        ...
        public IMounteaDialogueTickableObject
    {
        ...
        // are the BlueprintNativeEvent hooks each concrete node type overrides.
    };
    ```
    !!! info
...
Regardless of type, every node goes through the same lifecycle when the Dialogue Manager walks the graph:

1. **`PreProcessNode`** — called before the node is considered active; sets up anything the node needs before running.
2. **`EvaluateDecorators`** — checks the node's own Decorators, and the Graph's Decorators if `bInheritGraphDecorators` is true. If any Decorator fails, the node is skipped.
3. **`CanStartNode`** — a final validation check specific to the node type.
4. **`ProcessNode`** — the node's actual behaviour: showing dialogue text, starting a timer, jumping to another node, or (for Start Node) simply nothing.
5. **`CleanupNode`** — called once the node is done, to release anything it was holding onto.
